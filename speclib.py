...
def fit_mGaussian(x, y, cx0, cy0, FWHMG=0.15): 
    spec = Spectrum1D(flux=(y-1)*u.flx, spectral_axis=x*u.Angstrom)
    fit_dx = FWHMG/10
    SIGMA = FWHMG/(2*np.sqrt(2*np.log(2)))
    ...
    for ix, iy in zip(cx0, cy0):
        p_init += models.Gaussian1D(iy-1, ix, FWHMG, \
...
def fit_mVoigt(x, y, cx0, cy0, FWHMG=0.15, FWHML=0.1): 
    spec = Spectrum1D(flux=(y-1)*u.flx, spectral_axis=x*u.Angstrom)
    fit_dx = FWHMG/10
    fit_fwhm_G1, fit_fwhm_G2 = FWHMG*(0.8), FWHMG*(1.5)
    fit_fwhm_L1, fit_fwhm_L2 = FWHML*(0.8), FWHML*(1.5)
    # make fitting models with Voigt
    p_init = models.Const1D(amplitude=0, fixed={'amplitude':True})
    for ix, iy in zip(cx0, cy0):
        p_init += models.Voigt1D(ix, (iy-1), FWHML, FWHMG, \
...
def local_continuum(xp, yp, lower=0.9, upper=1.4, N_MAX=15):
    x0 = xp.mean() 
    xp = xp - x0
